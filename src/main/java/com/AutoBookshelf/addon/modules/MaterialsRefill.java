    ...
    private void placeShulker() {
        ...
        if (mc.crosshairTarget instanceof BlockHitResult blockHit) {
            ...
            if ((mc.world.getBlockState(candidate).isAir()
                ...
                && mc.player.getEntityPos().squaredDistanceTo(candidate.getX() + 0.5, candidate.getY() + 0.5, candidate.getZ() + 0.5) <= reach * reach) {
                placePos = candidate;
        ...
        if (placePos == null) {
            ...
            double reach = mc.player.getEntityInteractionRange();
            if (mc.player.getEntityPos().squaredDistanceTo(candidate.getX() + 0.5, candidate.getY() + 0.5, candidate.getZ() + 0.5) > reach * reach) {
                stage = Stage.IDLE;
                resetState();
                return;
            }
