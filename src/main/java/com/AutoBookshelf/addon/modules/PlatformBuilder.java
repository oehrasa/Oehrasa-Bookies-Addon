...
public class PlatformBuilder extends Module {
    ...
    public void onTick(TickEvent.Post event) {
        ...
        pendingPlacements.removeIf(pos -> needsBlock(mc.level.getBlockState(pos)));
        ...
        java.util.Arrays.sort(targets,
            Comparator.comparingDouble(a -> a.distSqr(origin)));
    ...
    private BlockPos findBaritoneEdgeSafeGoal() {
        ...
        for (BlockPos pos : areaSelector.getFlatArea(targetY)) {
            ...
            for (BlockPos n : new BlockPos[]{pos.north(), pos.south(), pos.east(), pos.west()}) {
                if (needsBlock(mc.level.getBlockState(n))) continue; // still an open gap, skip it

                double dist = n.distSqr(playerPos);
                if (dist < bestDist) {
                    bestDist = dist;
                    best = n;
                }
    ...
    private boolean ensureBuildMatInHand() {
        ...
        for (int i = 0; i < 9; i++) {
            if (isAllowedStack(mc.player.getInventory().getItem(i))) {
    ...
    private void tickShulkerCollection() {
        ...
        BlockPos itemPos = pendingShulkerItem.blockPosition();
    ...
    private ItemEntity findDroppedShulker() {
        ...
        for (Entity entity : mc.level.entitiesForRendering()) {
            ...
            if (!(Block.byItem(item.getItem().getItem()) instanceof ShulkerBoxBlock)) continue;
            if (isShulkerEmpty(item.getItem())) continue;
