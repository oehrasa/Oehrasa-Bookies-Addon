...
public class PlatformBuilder extends Module {
    ...
    private final ShulkerRestockEngine restockEngine = new ShulkerRestockEngine(mc, placementEngine,
        new ShulkerRestockEngine.RestockCallback() {
            ...
            public void onFinished(boolean success) {
                // ShulkerRestockEngine previously mis-tracked how many inventory
                // slots quick-moving the target item actually freed up (it assumed
                // every take consumed a slot, but a quick-move usually merges into
                // an existing partial stack of your build material instead). That
                // made it stop early "to keep 1 slot free" even when no slot had
                // really been used, leaving items behind in the shulker for no
                // reason. That's now fixed at the source, but flag it here too in
                // case it's still happening for a genuinely-out-of-room inventory.
                ...
                if (success && restockBreakAfterFill.get()) {
                    // The shulker was mined (not just closed) after refilling, so it's
                    // now a dropped item entity on the ground rather than safely back
                    // in a container. Don't just wait out the resume delay blindly -
                    // actively locate and walk to it so it's reliably collected before
                    // build resumes. Vanilla's pickup radius won't always catch it if
                    // the break knocked it a block or two away, and if it still has
                    // contents, vanilla can't merge it into anything and needs a truly
                    // empty slot to pick it up at all.
                    collectingDroppedShulker = true;
                    pendingShulkerItem = null;
                    shulkerCollectTimeout = 100;
                } else {
                    // Either way, building resumes on its own next tick: reachableTargets()
                    // and the Baritone goal both get re-evaluated fresh once the engine is idle.
                    // On success we still wait a few ticks (restockResumeDelay) before
                    // resuming placement - this gives the hotbar-slot restore in
                    // ShulkerRestockEngine time to fully settle server-side, instead of
                    // ensureBuildMatInHand() racing it and grabbing whatever's currently
                    // in-hand (which used to be the shulker box itself).
    ...
    public void onActivate() {
        // Deliberately NOT clearing areaSelector/buildStarted here: onActivate fires
        // again after a dimension change re-triggers the module lifecycle, and the
        // user's selection + build-in-progress intent should survive that. Only
        // runtime/per-world state (pending placements, Baritone goals, restock
        // progress) gets reset - see onDeactivate below for the matching half of this.
        ...
        if (useBaritone.get()) {
            try {
                ...
                // Baritone's assumeSafeWalk defaults to true, meaning it assumes some
                // external mechanic (a safewalk mod, Feather Falling, MLG, etc.) will
                // prevent fall damage, so it doesn't bother sneaking near ledges - it
                // just strides straight off the edge of the platform. We don't have
                // that mechanic here, so force it off (and remember/restore whatever
                // it was, since this is a global Baritone setting, not per-module).
    ...
    public void onDeactivate() {
        pendingPlacements.clear();
        // NOTE: areaSelector.reset() and buildStarted = false were removed from here.
        // This method fires on every dimension change (Meteor deactivates/reactivates
        // modules across world swaps), so wiping the selection here meant switching
        // dimensions silently threw away your platform selection and build progress.
        // Selection/build-started state now only clears via the explicit
        // resetSelectionKey handler in onInteract, or a genuine manual module toggle
        // off-then-on where the user re-selects anyway.
        //
        // If a restock was mid-flight when this fires (e.g. player forced through a
        // portal), make sure whatever hotbar slot the restock engine swapped to gets
        // handed back rather than left as the shulker box.
    ...
    private void onRender(Render3DEvent event) {
        // Always preview flattened at the current yLevel so what's shown is
        // exactly where the platform will be built - corners included.
    ...
    public void onTick(TickEvent.Post event) {
        ...
        if (moveUpKey.get().isPressed()) {
            ...
            // The old build was targeting the previous Y-level's flat area; letting
            // it keep running here would place blocks that don't match what you're
            // now looking at (and don't match the rendered selection anymore).
            if (buildStarted) cancelBuild("§cBuild cancelled – Y-level changed.");
        ...
        // Dropped-shulker collection (post-restock) owns the tick until the item
        // is gone or we give up - don't let placement/Baritone logic fight it.
        ...
        // Don't place a single block until the user explicitly confirms - selection
        // being complete just means pos1/pos2 are set, not that building should start.
        if (!buildStarted) {
        ...
        if (restockCooldown > 0) {
            restockCooldown--;
            // Still counting down the post-restock settle delay: deliberately return
            // here instead of falling through to ensureBuildMatInHand()/placement,
            // so we never race the hotbar-slot restore that ShulkerRestockEngine just
            // issued (that race was the root cause of the shulker-as-platform-block bug).
            return;
        ...
        if (targets.length == 0) {
            ...
            if (useBaritone.get() && baritone != null) {
                ...
                // If there's no safe, already-solid neighbor to stand on yet (e.g. the
                // very first cell of the build over open space), we deliberately don't
                // send Baritone anywhere - better to wait than have it walk off the edge.
            }
            return;
        }
    ...
    /**
     * Finds a target next to (not on top of) the closest incomplete platform cell.
     * We deliberately never path Baritone across the incomplete/missing area itself -
     * it's an open gap at this yLevel, and a plain XZ goal doesn't care about Y, so it
     * would happily send Baritone straight over the edge into the void. Instead we only
     * ever send it to a neighboring cell that's already solid (already placed, or
     * natural terrain), so it always has something to stand on right up to the edge.
     */
    ...
    private ShulkerRestockEngine.RestockConfig buildRestockConfig() {
        // The current build footprint (flattened at yLevel) is excluded from shulker
        // placement candidates, so the restock sequence can never place - or the
        // build logic mistake - the shulker box as if it were a platform block.
    ...
    private void tickShulkerCollection() {
        ...
        if (pendingShulkerItem == null || !pendingShulkerItem.isAlive()) {
            ...
            if (pendingShulkerItem == null) {
                // Nothing left on the ground - either vanilla proximity pickup
                // already grabbed it, or it despawned. Either way, done here.
                stopCollectingShulker();
                return;
            }
        }
    ...
    /**
     * Adapted from AutoRestock.findDroppedShulker: locates the nearest dropped
     * shulker-box item entity near the player, so the collection logic above has
     * something concrete to walk toward instead of assuming it self-collects.
     * <p>
     * Deliberately skips empty shulker boxes. A separate module in this setup
     * continuously discards empty shulkers of its own, so without this filter
     * PlatformBuilder could latch onto one of those unrelated boxes instead of
     * (or ahead of) the one it actually cares about - the one that still has
     * leftover build material in it from a restock attempt. Empty shulkers also
     * don't need active collection in the first place (see the isShulkerEmpty
     * check itself for why), so there's nothing lost by ignoring them here.
     */
