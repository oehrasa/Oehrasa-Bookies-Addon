...
public class ChatWindow extends LiveWindow {
    ...
    public void toggleColor() {
        ...
        if (this.chatSettings.customColor != 0) {
            for (int i = 0; i < MINECRAFT_COLOR_VALUES.length; i++) {
                if (this.chatSettings.customColor == MINECRAFT_COLOR_VALUES[i]) {
                    currentIndex = i;
                    break;
                }
            }
        ...
        if (currentIndex == MINECRAFT_COLOR_VALUES.length) {
            ...
            this.chatSettings.customColor = MINECRAFT_COLOR_VALUES[currentIndex];
    ...
    public void clampScrollPosition() {
        this.chatScrollPosition = Mth.clamp(this.chatScrollPosition, 0, this.getMaxScrollPosition());
    }
    ...
    private int getInputBoxHeight() {
        int lineCount = Mth.clamp(this.inputBox.wrappedLines(this.fontRenderer, this.w - 18).size(), 1, MAX_VISIBLE_INPUT_LINES);
    ...
    private boolean autocompletePlayerName() {
        ...
        if (partial.isEmpty() || this.mc.getConnection() == null) return false;

        String match = null;
        for (PlayerInfo entry : this.mc.getConnection().getOnlinePlayers()) {
        ...
        for (String rawLine : full.split("\n")) {
            ...
            for (String chunk : splitToFit(trimmedLine, this.maxLineLength)) {
                ...
                if (firstLine && online && !WhisperRateLimiter.isOnCooldown()) {
                    ...
                    this.mc.player.connection.sendCommand(LiveMessage.INSTANCE.getPmCommand() + " " + this.liveProfile.username + " " + chunk);
                    WhisperRateLimiter.recordSent();
    ...
    public void keyTyped(char typedChar, int keyCode) {
        ...
        if (keyCode == 257 || keyCode == 335) { // Enter / numpad Enter
            ...
            if (this.mc.keyboardHandler != null) {
                ...
                if (!copied.isEmpty()) this.mc.keyboardHandler.setClipboard(copied);
            }
            ...
            if (this.mc.keyboardHandler != null) {
                ...
                if (!cut.isEmpty()) this.mc.keyboardHandler.setClipboard(cut);
            }
    ...
    private void copyMessageToClipboard(String text) {
        ...
        if (this.mc.keyboardHandler != null) {
            this.mc.keyboardHandler.setClipboard(safe);
        }
    ...
    private boolean isShiftHeld() {
        return GLFW.glfwGetKey(this.mc.getWindow().handle(), GLFW.GLFW_KEY_LEFT_SHIFT) == 1
            || GLFW.glfwGetKey(this.mc.getWindow().handle(), GLFW.GLFW_KEY_RIGHT_SHIFT) == 1;
    }
    ...
    private void placeCursorFromMouse(int mouseX, int mouseY, boolean selecting) {
        ...
        int lineIdx = Mth.clamp(start + Math.floorDiv(relY, INPUT_LINE_HEIGHT), 0, lineCount - 1);
        this.inputBox.setCursorAt(this.fontRenderer, inputWidth, lineIdx, mouseX - this.x - 8, selecting);
        this.inputBox.ensureCaretVisible(this.fontRenderer, inputWidth);
    }

    private boolean isCtrlHeld() {
        return GLFW.glfwGetKey(this.mc.getWindow().handle(), GLFW.GLFW_KEY_LEFT_CONTROL) == 1
    ...
    private void drawChatHistory(GuiGraphicsExtractor context, int chatColorMe, int chatColorOther) {
