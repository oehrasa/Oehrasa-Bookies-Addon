...
public class ManeWindow extends LiveWindow {
    ...
    private void drawProfilePic(GuiGraphicsExtractor context, int x, int y, UUID uuid) {
        ...
        for (int i = listScrollPosition; i < buddyListEntries.size() && lineHeight < maxVisibleLines; i++) {
            ...
            if (buddyListEntry.uuid != null) {
                ...
                if (tabEntry != null) {
                    ...
                    PlayerFaceExtractor.extractRenderState(context, tabEntry.getSkin(), 10, yPos - 1, 10, -1);
                } else if (buddySkin.hasCachedSkin()) {
                    PlayerFaceExtractor.extractRenderState(context, buddySkin.getCachedTextures(), 10, yPos - 1, 10, -1);
                }
            }
    ...
    public void drawTextFields(GuiGraphicsExtractor context) {
        ...
        searchField.extractWidgetRenderState(context, this.lastMouseX - this.x, this.lastMouseY - this.y, 0.0F);
        context.pose().translate(-this.x, -this.y);
    }
