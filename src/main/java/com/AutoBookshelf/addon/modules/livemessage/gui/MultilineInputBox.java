...
public class MultilineInputBox {
    ...
    public void moveUp(Font renderer, int maxWidth) {
        this.beginMove();
    ...
    public void moveDown(Font renderer, int maxWidth) {
        this.beginMove();
        List<Segment> segs = this.wrapSegments(renderer, maxWidth);
        int lineIdx = this.cursorLineIndex(renderer, maxWidth);
     ...
     */
    public void setCursorAt(Font renderer, int maxWidth, int lineIndex, int pixelX, boolean selecting) {
        ...
        List<Segment> segs = this.wrapSegments(renderer, maxWidth);
        Segment seg = segs.get(Mth.clamp(lineIndex, 0, segs.size() - 1));
        this.cursor = Mth.clamp(seg.startOffset + columnAtPixel(renderer, seg.text, pixelX), 0, this.text.length());
        this.resetBlink();
    }

    private static int columnAtPixel(Font renderer, String line, int pixelX) {
        if (pixelX <= 0 || line.isEmpty()) return 0;
        for (int i = 1; i <= line.length(); i++) {
            int before = renderer.width(line.substring(0, i - 1));
            int after = renderer.width(line.substring(0, i));
    ...
    public List<Segment> wrapSegments(Font renderer, int maxWidth) {
    ...
    public List<String> wrappedLines(Font renderer, int maxWidth) {
        List<Segment> segs = this.wrapSegments(renderer, maxWidth);
    ...
    public int cursorLineIndex(Font renderer, int maxWidth) {
        List<Segment> segs = this.wrapSegments(renderer, maxWidth);
        int best = 0;
    ...
    public int cursorColumnInLine(Font renderer, int maxWidth) {
        List<Segment> segs = this.wrapSegments(renderer, maxWidth);
        int idx = this.cursorLineIndex(renderer, maxWidth);
        Segment s = segs.get(idx);
        return Mth.clamp(this.cursor - s.startOffset, 0, s.text.length());
    }
    ...
    public int getMaxScrollIndex(Font renderer, int maxWidth) {
        return Math.max(0, this.wrapSegments(renderer, maxWidth).size() - this.maxVisibleSegments);
    }

    public void clampScroll(Font renderer, int maxWidth) {
        this.scrollIndex = Mth.clamp(this.scrollIndex, 0, this.getMaxScrollIndex(renderer, maxWidth));
    }

    public void scrollBy(int lines, Font renderer, int maxWidth) {
        this.scrollIndex = Mth.clamp(this.scrollIndex + lines, 0, this.getMaxScrollIndex(renderer, maxWidth));
    }
     ...
     */
    public void ensureCaretVisible(Font renderer, int maxWidth) {
        ...
        this.scrollIndex = Mth.clamp(this.scrollIndex, 0, maxScroll);
    }
