...
public class NotesWindow extends LiveWindow {
    ...
    private void beginEdit(NoteEntry note) {
        ...
        this.inputField.setValue(note.subtext != null && !note.subtext.isEmpty() ? note.text + " | " + note.subtext : note.text);
        this.inputField.setFocused(true);
    }
    ...
    public void drawWindow(GuiGraphicsExtractor context, int bgColor, int fgColor) {
        ...
        if (this.notes.isEmpty()) {
            ...
            for (int i = this.scrollPosition; i < this.notes.size() && drawn < maxLines; i++, drawn++) {
                ...
                String clipped = this.fontRenderer.plainSubstrByWidth(note.text, this.w - 40 - timeWidth - 6);
                ...
                if (note.subtext != null && !note.subtext.isEmpty()) {
                    String clippedSub = this.fontRenderer.plainSubstrByWidth(note.subtext, this.w - 40);
