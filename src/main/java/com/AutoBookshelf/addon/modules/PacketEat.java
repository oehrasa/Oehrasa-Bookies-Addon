    ...
    private void handleAutoEat(ClientPlayerEntity player) {
        ...
        if (autoEating) {
            ...
            if (eatTicks >= EAT_DURATION_TICKS) {
                // Timed out with zero visible change... the interaction likely never landed.
                // Reset so the next tick can retry with a fresh sequence number.
                autoEating = false;
                eatTicks = 0;
                postEatCooldown = cooldownTicks.get();
            }
        ...
        // 5. Send the "start use" packet with a real
        player.networkHandler.sendPacket(
            new PlayerInteractItemC2SPacket(Hand.OFF_HAND, interactionSequence++, player.getYaw(), player.getPitch())
        );
