...
import com.AutoBookshelf.addon.mixin.PlayerInventoryAccessor;
...
public class AxolotlTools extends Module {
    public AxolotlTools() {
        super(Addon.CATEGORY, "AxolotlTools", "Extrasensory perception for axolotl variants, auto-collector & auto-breeder.");
    }
    ...
    private final Setting<AxolotlMode> axolotlMode = sgAuto.add(
        new EnumSetting.Builder<AxolotlMode>()
            ...
            .defaultValue(AxolotlMode.Catch)
            .onChanged(it -> {
    ...
    private final Setting<VariantBehavior> interactPink = sgVariantChoices.add(
        new EnumSetting.Builder<VariantBehavior>()
            ...
            .defaultValue(VariantBehavior.Esp)
            .onChanged(it -> {
    ...
    private void infoThrottled(String key, String message) {
        long now = System.currentTimeMillis();
        Long last = msgCooldowns.get(key);
        if (last == null || now - last >= MSG_COOLDOWN_MS) {
            info(message);
            msgCooldowns.put(key, now);
        }
    }
    ...
    private <T extends LivingEntity> boolean tryInteractMobFull(T entity, Item bucketType) {
        ...
        for (int n = 0; n < ((PlayerInventoryAccessor) mc.player.getInventory()).getMain().size(); n++) {
            ...
            Rotations.rotate(
                ...
                () -> result.set(mc.gameMode.interact(
                    mc.player,
                    entity,
                    new EntityHitResult(entity, entity.position()),
        ...
        infoThrottled("noBucketFound", "No valid bucket types found in inventory§c..!");
        return false;
    }
                            ...
                            if (axolotlMode.get() == AxolotlMode.Catch) {
                                if (fillBuckets.get() && hasNoValidBucket(Items.WATER_BUCKET) && current.getItem() != Items.BUCKET) {
                                    ...
                                    if (source != null && (hasEmptySlots() || current.getCount() == 1)) {
                                        ...
                                        infoThrottled("fullInventoryWarning", "Full inventory prevents auto-filling buckets§c..!");
                                    }
                                }
                            ...
                            if (hit.getEntity() instanceof Axolotl axolotl) {
                                if (axolotlMode.get() == AxolotlMode.Catch) {
                                    ...
                                    if (tryInteractMobTrigger(axolotl, Items.TROPICAL_FISH_BUCKET)) return;
                                        ...
                                        infoThrottled("breedingCooldownUpdate", "That axolotl isn't ready to eat yet§c..!");
                                    }
                                }
                            }
