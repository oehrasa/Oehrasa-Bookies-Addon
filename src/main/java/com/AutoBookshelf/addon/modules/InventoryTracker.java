...
public class InventoryTracker extends Module {
    ...
    private void checkHeldBook(UUID uuid, PlayerEntity player, ItemStack stack) {
    ...
    private void refreshPredictedScreen(List<String> oldPages, List<String> newPages, ItemStack stack) {
        if (!(mc.currentScreen instanceof BookScreen)) {
        ...
        mc.execute(() -> {
            BookScreen screen = new BookScreen(BookScreen.Contents.create(stack));
            mc.setScreen(screen);
    ...
    public static class TrackedInventory {
        ...
        private static boolean itemsEqualIgnoringDurability(ItemStack a, ItemStack b) {
            ...
            strippedA.remove(DataComponentTypes.DAMAGE);
            strippedB.remove(DataComponentTypes.DAMAGE);
            return ItemStack.areItemsAndComponentsEqual(strippedA, strippedB);
        }
