...
public class InventoryTracker extends Module {
    ...
    public final Map<UUID, Player> playerMap = new HashMap<>();
    ...
    private void onTick(TickEvent.Post event) {
        ...
        for (Player player : mc.level.players()) {
            if (!(player instanceof RemotePlayer)) continue;
            ...
            for (int i = 0; i < current.length; i++) {
                if (ItemStack.isSameItemSameComponents(current[i], last[i])) continue;
                ...
                ItemStack mainHandStack = (slot == EquipmentSlot.MAINHAND) ? newStack : player.getMainHandItem();
    ...
    private void injectTrackedItems(Inventory inventory, TrackedInventory tracked, ItemStack mainHand) {
        ...
        for (int i = start; i <= end; i++) {
            ...
            inventory.getNonEquipmentItems().set(i + offset, fill);
        }
    }

    private ItemStack[] snapshotEquipment(Player player) {
    ...
    private void reset() {
        if (mc.level != null) {
            Map<UUID, Player> live = new HashMap<>();
            for (Player p : mc.level.players()) live.put(p.getUUID(), p);

            for (UUID uuid : inventoryMap.keySet()) {
                Player player = live.get(uuid);
                if (player == null) continue;
                Inventory inv = player.getInventory();
                for (int i = 1; i < inv.getNonEquipmentItems().size(); i++) {
    ...
    private void buildRenderPreview(UUID uuid, Player player, TrackedInventory tracked) {
        ...
        Component nameText = Component.literal(player.getName().getString());
    ...
    private void drawTrackedPanel(GuiGraphicsExtractor context, PreviewData preview, int panelX, int panelY) {
        int pad = 2;
        ...
        context.text(mc.font, preview.nameText(), panelX + pad, textY, 0xFFFFFFFF);
        textY += 10;
