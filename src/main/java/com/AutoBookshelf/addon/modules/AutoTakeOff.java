...
public class AutoTakeOff extends Module {
    ...
    private void applySimplePitch() {
        if (simpleSetPitch.get()) {
            mc.player.setXRot((float) simpleTakeoffPitch.get().doubleValue());
        }
    }
    ...
    private void fireRocket() {
        ...
        if (rocketSlot == -1) {
            if (mc.player.getOffhandItem().getItem() instanceof FireworkRocketItem) {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (waitingForGlide > 0) {
            waitingForGlide--;
            if (setPitch.get() && mode.get() == Mode.Normal && !mc.player.isFallFlying()) {
                holdTakeoffRotation();
            }
        ...
        if (mode.get() == Mode.Simple) {
            // Delay/deploy countdown from the initial ground jump.
            if (simpleJumped) {
                if (takeOffDelay > 0) {
                    takeOffDelay--;
                    if (takeOffDelay == 0) {
            ...
            // Ground takeoff: initial jump, starts the delay above
            ...
            // Falling takeoff
            if (takeOffWhenFalling.get() && mc.player.getDeltaMovement().y < fallingVelocityThreshold.get()) {
                applySimplePitch();
                mc.player.jumpFromGround();
                sendStartFlyingPacket();
                cooldownTimer = cooldown.get();
