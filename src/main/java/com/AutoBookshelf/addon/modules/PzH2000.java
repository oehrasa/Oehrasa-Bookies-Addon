...
public class PzH2000 extends Module {
    ...
    private List<Vec3d> cachedPath = new ArrayList<>();
    ...
    private Vec3d pendingShooterPos;
    ...
    private void onRender(Render3DEvent event) {
        ...
        boolean drawing = mc.options.useKey.isPressed() || (autoFire.get() && mc.player.isUsingItem());
    ...
    private void onTick(TickEvent.Post event) {
        ...
        // early return, or a purely-manual release (which drops mc.options.useKey.isPressed()
        ...
        // Live aim: solved for the ACTUAL current draw (mc.player.getItemUseTime()),
        int currentUseTicks = isUsingNow ? mc.player.getItemUseTime() : 0;
        double currentSpeed = 3.0 * BowItem.getPullProgress(currentUseTicks);
    ...
    private int ticksForSpeed(double requiredSpeed) {
        for (int ticks = minChargeTicks.get(); ticks <= maxChargeTicks.get(); ticks++) {
            double speed = 3.0 * BowItem.getPullProgress(ticks);
    ...
    private void recordShotOutcome(Vec3d actualFinalPos) {
        ...
        Vec3d error = toActual.subtract(forward.multiply(toActual.dotProduct(forward)));
        double lateralError = error.dotProduct(right);
        double verticalError = error.dotProduct(up);
    ...
    private boolean hasLineOfSight(Vec3d from, Vec3d to) {
        BlockHitResult hit = mc.world.raycast(new RaycastContext(
            from, to, RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.NONE, mc.player
        ));
    ...
    private void renderPrediction(Render3DEvent event) {
        ...
        if (cachedSolutionFound && cachedPath.size() > 1) {
            for (int i = 0; i < cachedPath.size() - 1; i++) {
                Vec3d a = cachedPath.get(i), b = cachedPath.get(i + 1);
    ...
    private ArcResult simulateArc(Vec3d startPos, Vec3d startVel, Vec3d targetPos, double targetDist3D, List<Vec3d> outPath) {
        ...
        for (int i = 0; i < simulationTicks.get(); i++) {
            ...
            BlockHitResult blockHit = mc.world.raycast(new RaycastContext(
                pos, next, RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.NONE, mc.player
            ));
