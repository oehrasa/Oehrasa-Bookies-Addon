...
public class DistanceUtil {
    ...
    // Vec3d + raw doubles overload, to avoid allocating a Vec3d at call sites that
    // only have loose x/y/z (e.g. world-space label positions).
    public static double distanceSq(Vec3d a, double x, double y, double z) {
        return distanceSq(a.x, a.y, a.z, x, y, z);
    }

    public static double distance(Vec3d a, double x, double y, double z) {
        return Math.sqrt(distanceSq(a, x, y, z));
    }

    // Vec2f overloads (Vec2f's fields are x/y, not x/z, so this is a flat 2D calc, not xz)
    public static double distanceSq(Vec2f a, Vec2f b) {
        double dx = a.x - b.x;
        double dy = a.y - b.y;
        return dx * dx + dy * dy;
    }
    ...
    public static double distanceSqToBox(Vec3d p, double minX, double minY, double minZ,
                                         double maxX, double maxY, double maxZ) {
