...
public class MapViewer extends HudElement {
    ...
    private MapId lastMapId1 = null, lastMapId2 = null;
    private boolean lastShowSecond = false;

    @Nullable
    private MapId mapId1, mapId2;
    @Nullable
    private MapItemSavedData mapData1, mapData2;
    ...
    public void tick(HudRenderer renderer) {
        ...
        MapId newId1 = null, newId2 = null;
        boolean newShowSecond = false;

        switch (mode.get()) {
            case Held -> {
                stack1 = mc.player.getMainHandItem();
                stack2 = mc.player.getOffhandItem();
            ...
            case Offhand -> {
                stack1 = mc.player.getOffhandItem();
                newId1 = getMapId(stack1);
            }
            case ItemFrame -> {
                if (mc.hitResult instanceof EntityHitResult entityHit
                    && entityHit.getEntity() instanceof ItemFrame frame) {
                    stack1 = frame.getItem();
                    newId1 = getMapId(stack1);
                }
            }
            case SlotIndex -> {
                stack1 = mc.player.getInventory().getItem(slotIndex.get());
                newId1 = getMapId(stack1);
            }
            case MapId -> {
        ...
        if (changed) {
            ...
            mapData1 = (mapId1 != null) ? MapItem.getSavedData(mapId1, mc.level) : null;
            mapData2 = (mapId2 != null) ? MapItem.getSavedData(mapId2, mc.level) : null;
        }
    ...
    private void drawMap(HudRenderer renderer, MapId id, MapItemSavedData data,
        ...
        renderer.post(() -> {
            ...
            var matrices = renderer.graphics.pose();
            matrices.pushMatrix();
    ...
    private boolean isMap(ItemStack stack) {
        return stack.is(Items.FILLED_MAP) && stack.has(DataComponents.MAP_ID);
    }

    private MapId getMapId(ItemStack stack) {
        return stack.get(DataComponents.MAP_ID);
    }

    private boolean areMapsDifferent(ItemStack a, ItemStack b) {
        MapId idA = getMapId(a);
        MapId idB = getMapId(b);
