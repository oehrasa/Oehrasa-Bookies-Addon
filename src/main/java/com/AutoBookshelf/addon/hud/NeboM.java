...
public class NeboM extends HudElement {
    ...
    private final Map<UUID, Identifier> headTextureCache = new HashMap<>();
    ...
    @Override public void render(HudRenderer renderer) {
        ...
        for (PlayerEntity player : getNearbyPlayers()) {
            ...
            // Draw head icon
            if (showHead.get()) {
                Identifier headTexture = getHeadTexture(player);
                if (headTexture != null) {
                    // Draw the head texture (configurable size)
                    // Fixed: pass the texture identifier to getGuiTextured
                    PlayerSkinDrawer.draw(
                        renderer.drawContext,
                        headTexture,
                        (int) x, (int) y - headSize.get() / 2,
                        headSize.get(),
                        true, false, Color.WHITE.getPacked()
                    );
                    x += headSize.get() + 2; // shift x right for text
                } else {
                    x += headSize.get() + 2; // still shift for missing head
                }
            }
    ...
    private Identifier getHeadTexture(PlayerEntity player) {
